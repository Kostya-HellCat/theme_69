	...
	*/
	...
	function render_attachment( $id = 0, $field ) {
		...
		<div class="acf-gallery-side-info">
			<img src="<?= esc_attr($thumb); ?>" alt="<?= esc_attr($attachment['alt']); ?>" />
			<p class="filename"><strong><?= esc_html($attachment['filename']); ?></strong></p>
			<p class="uploaded"><?= esc_html($attachment['dateFormatted']); ?></p>
			<p class="dimensions"><?= esc_html($dimensions); ?></p>
			<p class="actions">
				<a href="#" class="acf-gallery-edit" data-id="<?= esc_attr($id); ?>"><?php _e('Edit', 'acf'); ?></a>
				<a href="#" class="acf-gallery-remove" data-id="<?= esc_attr($id); ?>"><?php _e('Remove', 'acf'); ?></a>
			</p>
		</div>
		...
		<div class="acf-gallery-attachments">
			<?php if( $attachments ): ?>
				<?php foreach( $attachments as $i => $attachment ): 
					...
					?>
					<div class="<?= esc_attr($a_class); ?>" data-id="<?= esc_attr($a_id); ?>">
						<input type="hidden" name="<?= esc_attr($field['name']); ?>[]" value="<?= esc_attr($a_id); ?>" />
						<div class="margin">
							<div class="thumbnail">
								<img src="<?= esc_url($a_thumbnail['url']); ?>" alt="" />
							</div>
							<?php if( $a_type !== 'image' ): ?>
								<div class="filename"><?= acf_get_truncated( $a_filename, 30 ); ?></div>	
							<?php endif; ?>
						</div>
						<div class="actions">
							<a class="acf-icon -cancel dark acf-gallery-remove" href="#" data-id="<?= esc_attr($a_id); ?>" title="<?php _e('Remove', 'acf'); ?>"></a>
						</div>
					</div>
				<?php endforeach; ?>
			<?php endif; ?>
