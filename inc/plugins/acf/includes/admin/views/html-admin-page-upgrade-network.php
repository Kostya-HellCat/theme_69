...
<div id="acf-upgrade-wrap" class="wrap">
	...
	<p><?= sprintf( __("The following sites require a DB upgrade. Check the ones you want to update and then click %s.", 'acf'), '"' . __('Upgrade Sites', 'acf') . '"'); ?></p>
		...
		foreach( $sites as $i => $site ): 
			...
			<tr<?php if( $i % 2 == 0 ): ?> class="alternate"<?php endif; ?>>
				...
				<?php if( acf_has_upgrade() ): ?>
					<input type="checkbox" value="<?= $site['blog_id']; ?>" name="checked[]">
				<?php endif; ?>
				</th>
				<td>
					<strong><?= get_bloginfo('name'); ?></strong><br /><?= home_url(); ?>
				</td>
				<td>
				<?php if( acf_has_upgrade() ): ?>
	...
	<p class="show-on-complete"><?= sprintf( __('Database Upgrade complete. <a href="%s">Return to network dashboard</a>', 'acf'), network_admin_url() ); ?></p>
